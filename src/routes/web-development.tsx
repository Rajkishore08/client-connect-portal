...
function WebDevelopmentPage() {
  const [activeTechTab, setActiveTechTab] = useState<keyof typeof TECH_STACK>("Languages");

  return (
    <main className="mx-auto max-w-7xl space-y-16 px-4 py-10 sm:px-6 sm:py-16">
      
      {/* Hero Banner Section (Light Theme) */}
      <header className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50/80 via-white to-slate-50/90 text-slate-900 p-6 sm:p-12 lg:p-14 shadow-sm border border-slate-200/90">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {OFFERINGS.map((offering) => {
            ...
            return (
              <div
                key={offering.title}
                className="group p-7 rounded-3xl bg-white border border-slate-200/90 shadow-sm hover:shadow-md hover:border-blue-400/50 transition-all space-y-4"
              >
                <div className={`h-12 w-12 rounded-2xl border ${offering.color} grid place-items-center transition-transform group-hover:scale-105`}>
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-bold text-slate-900">{offering.title}</h3>
                <p className="text-xs leading-relaxed text-slate-600">{offering.description}</p>
              </div>
            );
          })}
        </div>
