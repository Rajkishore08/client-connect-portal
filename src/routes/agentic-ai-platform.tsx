  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 pt-24 sm:pt-32 pb-16 space-y-16 sm:space-y-24">
      ...
      <section className="grid gap-8 lg:grid-cols-2">
        {/* Admin Hub */}
        <div className="p-8 rounded-3xl bg-white text-slate-900 space-y-6 border border-slate-200/90 shadow-sm">
          <div className="space-y-2">
            <span className="text-[11px] font-mono font-extrabold uppercase text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-200 inline-block">
              🗂️ ADMIN HUB
            </span>
            <h3 className="text-2xl font-black text-slate-900">Build, Deploy &amp; Govern Agents — No Code</h3>
          ...
          <div className="space-y-4">
            {ADMIN_HUB_FEATURES.map((f) => (
              <div key={f.title} className="p-4 rounded-2xl bg-slate-50 border border-slate-200/90 space-y-1">
                <h4 className="text-xs font-extrabold text-slate-900 flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-blue-600" /> {f.title}
                </h4>
      ...
      <section className="rounded-3xl bg-white text-slate-900 p-8 sm:p-12 space-y-8 border border-slate-200/90 shadow-sm">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <h2 className="text-3xl font-black sm:text-4xl text-slate-900">Enterprise-Grade AI Governance Built-In Day One</h2>
        </div>

        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {ENTERPRISE_SECURITY.map((sec) => (
            <div key={sec.title} className="p-6 rounded-2xl bg-slate-50 border border-slate-200/90 space-y-2">
              <h4 className="text-sm font-extrabold text-slate-900 flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-blue-600" /> {sec.title}
              </h4>
              <p className="text-xs text-slate-600 leading-relaxed font-medium">{sec.desc}</p>
            </div>
          ))}
        </div>

        <div className="flex flex-wrap justify-center items-center gap-3 pt-4 text-xs font-extrabold text-slate-700">
          <span className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200">SOC 2 Type II</span>
          <span className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200">ISO 27001</span>
          <span className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200">GDPR Compliant</span>
          <span className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200">AES-256 Encrypted</span>
          <span className="px-4 py-2 rounded-xl bg-slate-100 border border-slate-200">Private Deployment</span>
        </div>
      </section>
