...
import { LightImpactCardsSection } from "./LightImpactCardsSection";
    ...
    <div className="space-y-8">
      <header className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50/80 via-white to-slate-50/90 text-slate-900 p-6 sm:p-10 lg:p-12 shadow-sm border border-slate-200/90">
        ...
        <div className="relative z-10 grid gap-8 lg:grid-cols-12 lg:items-center">
          ...
          {/* Right Interactive 3D Robot Showcase (Light Theme Card matching Image 1 with Top Text) */}
          <div className="lg:col-span-5 relative w-full h-[420px] sm:h-[480px] rounded-3xl overflow-hidden border border-slate-200/90 shadow-xl bg-gradient-to-b from-white via-slate-50 to-slate-100">
            <RobotHero
              hideNavbar={true}
              backgroundText="AGENTS"
              pantallaColor="#00ffc6"
              pantallaBrillo={1.4}
              color="#d1d5db"
              metalness={0.1}
            />
            <div className="absolute bottom-3 left-3 right-3 bg-white/90 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-slate-200 shadow-md text-center pointer-events-none z-30">
              <p className="text-[11px] font-bold text-slate-800 flex items-center justify-center gap-1.5">
      ...
      {/* Light Impact Cards Section (Matching Image 2 in Light Theme) */}
      <LightImpactCardsSection />
    </div>
  );
}
