import { Bot, Code2, MessageCircle, RotateCcw, Send, ShieldCheck, Sparkles, TrendingUp, X } from "lucide-react";
...
const GREETING: ChatMessage = {
  id: "greeting",
  role: "assistant",
  text: "Hello! I am the **One World Solutions AI Concierge**.\n\nSelect a division below (**Web Development**, **Digital Marketing**, or **Passport Services**) or ask any question for an instant in-depth technical analysis.",
};
    ...
    <div className="space-y-1.5 font-sans leading-relaxed text-xs">
      {lines.map((line, lineIdx) => {
        ...
        if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
          ...
          return (
            <div key={lineIdx} className="flex items-start gap-2 pl-1 py-0.5 text-slate-800">
              <span className="h-1.5 w-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
              <div className="flex-1">{formattedBullet}</div>
            </div>
          );
        }

        // Numbered list format (e.g. 1., 2.)
        if (/^\d+\.\s/.test(trimmed)) {
          ...
          if (match) {
            ...
            const formattedNum = numParts.map((part, pIdx) => {
              if (part.startsWith("**") && part.endsWith("**")) {
                return (
                  <strong key={pIdx} className="font-bold text-slate-900">
                    {part.slice(2, -2)}
            ...
            return (
              <div key={lineIdx} className="flex items-start gap-2 pl-0.5 py-0.5 text-slate-800">
                <span className="font-bold text-primary font-mono text-[11px] shrink-0">{num}</span>
                <div className="flex-1">{formattedNum}</div>
              </div>
            );
          }
        }

        return <div key={lineIdx}>{formattedContent}</div>;
      })}
    </div>
  );
}
...
export function ChatWidget() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed z-50 flex flex-col bg-white overflow-hidden text-slate-900 font-sans transition-all duration-500 ease-[cubic-bezier(0.34,1.25,0.64,1)] origin-bottom-right ${
          open
        ...
        <div className="space-y-3 border-t border-slate-100 bg-white p-3.5">
          ...
          <div className="flex items-center gap-1 border-b border-slate-100 pb-2 overflow-x-auto no-scrollbar">
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-extrabold whitespace-nowrap transition-all cursor-pointer flex items-center gap-1.5 ${
                activePillar === "web"
                  ? "bg-blue-600 text-white shadow-xs"
                  : "bg-slate-100 text-slate-600 hover:bg-slate-200"
              }`}
            >
              <Code2 className="h-3.5 w-3.5 shrink-0" /> Web Dev &amp; AI
            </button>
            <button
              type="button"
